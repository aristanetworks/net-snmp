...
#include <net-snmp/library/snmpTCPNS6Domain.h>
...
static netsnmp_tdomain tcpns6Domain;
...
static char *
netsnmp_tcpns6_fmtaddr(netsnmp_transport *t, void *data, int len)
{
    return netsnmp_ipv6_fmtaddr("TCPNS/IPv6", t, data, len);
}

static int
netsnmp_tcpns6_accept(netsnmp_transport *t)
{
    ...
    if (farend == NULL) {
         ...
         */
        DEBUGMSGTL(("netsnmp_tcpns6", "accept: malloc failed\n"));
        return -1;
    }
    ...
    if (t != NULL && t->sock >= 0) {
        ...
        if (newsock < 0) {
            DEBUGMSGTL(("netsnmp_tcpns6","accept failed rc %d errno %d \"%s\"\n",
        ...
        str = netsnmp_tcpns6_fmtaddr(NULL, farend, farendlen);
        DEBUGMSGTL(("netsnmp_tcpns6", "accept succeeded (from %s)\n", str));
        free(str);
...
netsnmp_tcpns6_transport(struct sockaddr_in6 *addr, const char *ns, int local)
{
    ...
    str = netsnmp_tcpns6_fmtaddr(NULL, (void *)addr, sizeof(struct sockaddr_in6));
    DEBUGMSGTL(("netsnmp_tcpns6", "open %s %s\n", local ? "local" : "remote", str));
    free(str);
    ...
    t->domain = netsnmpTCPNS6Domain;
    t->domain_length = netsnmpTCPNS6Domain_len;

    DEBUGMSGTL(("netsnmp_tcpns6_transport", "Opening TCP socket in namespace %s...\n", ns));
    t->sock = netsnmp_open_namespace_socket(PF_INET6, SOCK_STREAM, 0, ns);
    ...
    if (local) {
        ...
        /* Try to restrict PF_INET6 socket to IPv6 communications only. */
        {
          ...
          if (setsockopt(t->sock, IPPROTO_IPV6, IPV6_V6ONLY, (char *)&one, sizeof(one)) != 0) {
            DEBUGMSGTL(("netsnmp_tcpns6", "couldn't set IPV6_V6ONLY to %d bytes: %s\n", one, strerror(errno)));
          } 
        }
#endif
        ...
        DEBUGMSGTL(("netsnmp_tcpns6", "connect returns %d\n", rc));
    ...
    t->f_accept   = netsnmp_tcpns6_accept;
    t->f_fmtaddr  = netsnmp_tcpns6_fmtaddr;
...
void
netsnmp_tcpns6_ctor(void)
{
    tcpns6Domain.name = netsnmpTCPNS6Domain;
    tcpns6Domain.name_length = netsnmpTCPNS6Domain_len;
